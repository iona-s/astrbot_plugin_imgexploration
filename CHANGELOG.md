...
- SauceNAO 返回结果被相似度阈值全部过滤时，在命令会话内发送提示，不再静默跳过该策略

### 🔄 变更

- 增强各搜图策略的结果数量日志，并记录 SauceNAO 候选、阈值过滤和最终返回数量
- 精简插件发布包，排除测试、CI 和本地开发配置文件

### 🐛 修复

- 修复 LLM 搜图在未明确选图或稳定图片 ID 失效时自动回退到其他图片的问题
- 修复 Ascii2d 结果页请求失败被误报为零匹配的问题；单个结果页成功时仍保留可用结果

## [1.1.2] - 2026-08-12

### ✨ 新增

- 支持QQ回复他人消息时自动插入@情况下识别 `/搜图` 命令

### 🔄 变更

- 自动图片上下文搜索默认忽略表情图片，仅在用户明确要求时搜索表情
